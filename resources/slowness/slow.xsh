...
def cpu_slow(secondary_server_config, secondaryip, secondarypids):
    ...
    ssh -i ~/.ssh/id_rsa @(secondaryip) @("sudo sh -c 'sudo echo {} > /sys/fs/cgroup/cpu/db/cpu.cfs_quota_us'".format(quota))
    ssh -i ~/.ssh/id_rsa @(secondaryip) @("sudo sh -c 'sudo echo {} > /sys/fs/cgroup/cpu/db/cpu.cfs_period_us'".format(period))
    
    for secondarypid in secondarypids.split():
        ssh -i ~/.ssh/id_rsa @(secondaryip) @("sudo sh -c 'sudo echo {} > /sys/fs/cgroup/cpu/db/cgroup.procs'".format(secondarypid))

def cpu_contention(secondary_server_config, secondaryip, secondarypids):
    cpu = secondary_server_config['cpu']
    scp resources/slowness/deadloop @(secondaryip):~/
    ssh -i ~/.ssh/id_rsa @(secondaryip) f"sh -c 'nohup taskset -ac {cpu} ./deadloop > /dev/null 2>&1 &'"
    ...
    ssh -i ~/.ssh/id_rsa @(secondaryip) @("sudo sh -c 'sudo echo {} > /sys/fs/cgroup/cpu/cpuhigh/cgroup.procs'".format(deadlooppid))

    for secondarypid in secondarypids.split():
        ssh -i ~/.ssh/id_rsa @(secondaryip) @("sudo sh -c 'sudo echo {} > /sys/fs/cgroup/cpu/cpulow/cgroup.procs'".format(secondarypid))
...
def disk_slow(secondary_server_config, secondaryip, secondarypids):
    ...
    for secondarypid in secondarypids.split():
        ssh -i ~/.ssh/id_rsa @(secondaryip) @("sudo sh -c 'sudo echo {} > /sys/fs/cgroup/blkio/db/cgroup.procs'".format(secondarypid))

def disk_contention(secondary_server_config, secondaryip, secondarypids):
    ssh -i ~/.ssh/id_rsa @(secondaryip) "sh -c 'nohup taskset -ac 2 ./clear_dd_file.sh > /dev/null 2>&1 &'"

def network_slow(secondary_server_config, secondaryip, secondarypids):
    ssh -i ~/.ssh/id_rsa @(secondaryip) "sudo sh -c 'sudo /sbin/tc qdisc add dev eth0 root netem delay 400ms'"

def memory_contention(secondary_server_config, secondaryip, secondarypids):
    set -ex

    ssh -i ~/.ssh/id_rsa @(secondaryip) "sudo sh -c 'sudo mkdir /sys/fs/cgroup/memory/db'"
    ...
    ssh -i ~/.ssh/id_rsa @(secondaryip) "sudo sh -c 'sudo echo 47088768 > /sys/fs/cgroup/memory/db/memory.limit_in_bytes'"   # 5MB
    
    for secondarypid in secondarypids.split():
        ssh -i ~/.ssh/id_rsa @(secondaryip) @("sudo sh -c 'sudo echo {} > /sys/fs/cgroup/memory/db/cgroup.procs'".format(secondarypid))
